...
Folder削除はsubtree削除になるため、`-r`、`-R`、または `--recursive` を必須にします。

Chrome Bookmarks APIへの書き込みはInfrastructure層で実行します。

Application層は対象解決、入力検証、Port経由の書き込み実行を扱います。

`rm` の対象がBookmarkの場合はInfrastructure層でBookmarks APIの `remove` を呼びます。

`rm` の対象がfolderの場合はInfrastructure層でBookmarks APIの `removeTree` を呼びます。
