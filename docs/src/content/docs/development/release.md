...
13. `pnpm run zip:amo-sources -- X.Y.Z` を実行する
14. Chrome zip、Firefox zip、AMO source package zipだけをrelease assetとして集める
15. docs archiveの差分があればmainへcommitしてpushする
16. release対象commitを解決する
17. `vX.Y.Z` のtagがなければ作成してpushする
18. `vX.Y.Z` のGitHub Release draftを作成または更新する

既存のrelease draftがある場合は、zip assetを上書きuploadします。

release draftがない場合は、`vX.Y.Z` のrelease draftを作成します。

AMOの「ソースコードをアップロード」欄には、`bookmark-cli-extention-X.Y.Z-amo-sources.zip` を使います。

このzipはFirefox buildを再現するために必要なroot設定、`src/`、`public/`、lockfile、AMO review notesだけを含めます。

docs site、archived docs、`dist/`、WXTが生成する大きい `sources.zip` はrelease assetに含めません。
...
Firefox Add-onsの審査用source packageは `pnpm run zip:amo-sources -- X.Y.Z` で生成します。
