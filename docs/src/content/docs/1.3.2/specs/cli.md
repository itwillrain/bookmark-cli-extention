...
入力済みcommandがある場合、`Tab` と `Shift+Tab` はfloating補完候補を優先し、古い結果一覧の行選択へは戻りません。
...
`/` はブラウザ管理root containerを隠した仮想rootを表します。

Chromeの `Bookmarks Bar` はCLI root `/` として扱います。

Chromeの `Other Bookmarks`、`Mobile Bookmarks`、`Managed Bookmarks` などは、CLI root直下のfolderとして表示します。

Firefoxなど `folderType` がない環境では、browser root直下containerの先頭をCLI root `/` として扱い、他のroot直下containerをCLI root直下のfolderとして表示します。

CLI rootへの書き込みでは、browser root nodeではなくroot保存用containerを `parentId` として使います。

Chromeの通常treeでは `Bookmarks Bar` がroot保存用containerです。
...
入力済みcommandがある場合、TabキーやShift+Tabキーは結果一覧ではなくfloating補完候補を優先します。
...
`rm` は削除対象のBookmark行を結果一覧として再表示しません。
...
`mkdir /Project` のようにCLI root直下へ作成する場合、Chrome Bookmarks APIへroot node IDを渡さず、root保存用containerへ作成します。
