...
slug: 1.3.2/specs/storage-permissions
---
...
v1ではpromptの表示スタイル、Nerd Font利用方針、command alias、command abbreviationを保存対象にします。
...
`commandAliases` はユーザー定義command aliasです。

保存経路はPopup設定画面と疑似CLIの `alias` / `unalias` です。

aliasは先頭command tokenにだけ実行時に1回展開します。

`commandAbbreviations` はユーザー定義command abbreviationです。

保存経路は疑似CLIの `abbr` / `unabbr` です。
...
`copy` commandはDedicated extension page上のBrowser Clipboard APIでtextを書き込みます。

v1.3.0では、manifestへ `clipboardWrite` permissionを追加しません。
