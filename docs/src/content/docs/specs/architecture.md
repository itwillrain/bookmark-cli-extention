...
コマンド入力欄、候補表示、Powerline風の結果一覧、plain text fallback、キーバインドを扱います。
...
5. Domain層の純粋関数で検索、path解決、previewを生成する
6. use caseが必要なChrome書き込み操作をPort経由で実行する
7. use caseがCommandResultを返す
8. Presentation層がCommandResultをPowerline風表示またはplain textへ変換する
...
background entrypointは、hot key、popup、拡張actionからCLIを開くときに起動元タブのsnapshotを取得します。
...
`mv`、`rm`、`rename` はpreview生成と実行を分けます。

preview生成はDomain層の純粋関数として扱います。

Chrome Bookmarks APIへの書き込みはInfrastructure層で実行します。

Application層は `--preview`、`--yes`、確認状態を見て、書き込みを行うかを決めます。
...
Powerline風segment、Nerd Font互換icon、CSS class名はPresentation層で扱います。
