...
v1ではpromptの表示スタイル、Nerd Font利用方針、command abbreviationを保存対象にします。
...
`commandAliases` はユーザー定義command abbreviationです。

保存経路はPopup設定画面と疑似CLIの `abbr` / `unabbr` です。

`alias` / `unalias` はv1.3.0以降も互換入力として受け付けます。

abbreviationは先頭command tokenにだけ1回展開します。
