...
14. ユーザーが `j Work` をEnterで確定する
15. 疑似CLIが入力を `jump Work` に展開して表示、実行、履歴保存する
16. ユーザーが疑似CLIで `unalias g` または `unabbr j` を実行する
17. 疑似CLIが対応する設定を削除して保存する
...
abbreviationは `settings.commandAbbreviations` に保存し、Enter確定時に入力欄とtranscriptの実行commandへ展開します。
