...
主な対象はhot key設定とcommand abbreviation設定です。
...
8. ユーザーがPopupでcommand abbreviationを追加、編集、削除する
9. Popupがabbreviationを `chrome.storage.local` の `settings.commandAliases` に保存する
10. ユーザーが疑似CLIで `abbr g=go` を実行する
11. 疑似CLIがabbreviationを `settings.commandAliases` に保存する
12. ユーザーが疑似CLIで `unabbr g` を実行する
13. 疑似CLIがabbreviationを削除して保存する

完了条件は、Popupで現在のhot keyを確認でき、ブラウザ標準のshortcut管理UIへ移動でき、Popupと疑似CLIの両方からcommand abbreviationを保存できることです。
