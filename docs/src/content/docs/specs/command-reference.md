...
| `doctor` | Bookmark Treeの整理候補を検出する      | 対象 |
| `dupes`  | 重複Bookmarkを検出する                 | 対象 |
...
Bookmark Treeの整理候補を検出します。

初期実装では、空titleのBookmark、同じURLを持つBookmark、同じtitleを持つBookmarkを検出します。
...
doctor
doctor --empty-title
doctor --duplicate-url | grep stripe
```

`doctor` のdefaultは `--empty-title` と `--duplicate-url` を対象にします。

`--duplicate-title` は同じtitleのBookmarkを多く検出する可能性があります。

明示指定または `--all` のときだけ対象にします。

結果は通常のBookmark resultとして表示し、直前結果一覧にも保存します。

そのため、診断結果の番号を使って `go 1`、`copy --url 1`、`rm 2` のように次の操作へ繋げられます。

空titleのBookmarkは、表示上だけ `(empty title)` として描画します。
...
dupes
dupes --title
dupes --all | grep admin
```

`dupes` のdefaultは `--url` です。

`--title` は空titleを除外し、trim後に大文字小文字を区別せず同じtitleになったBookmarkを検出します。

`--all` はURL重複とtitle重複の両方を表示します。
