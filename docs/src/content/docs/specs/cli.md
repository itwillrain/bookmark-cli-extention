...
Chromeの `Bookmarks Bar` / `Other Bookmarks` は、CLI上のfolderとして表示しません。

Firefoxの `ブックマークメニュー` / `ブックマークツールバー` / `他のブックマーク` も同じ扱いです。

root直下containerの子folderとBookmarkを、CLIの `/` 直下のentryとして扱います。
...
Tree guideと通常resultの読み始めを揃えるため、結果行ではfaviconやiconをtitleの前に表示しません。

Faviconやiconを表示する場合は、`tree` 以外の結果行でもtitleやURLの後ろに表示します。
