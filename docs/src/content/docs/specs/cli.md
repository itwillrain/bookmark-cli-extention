...
結果一覧のBookmark候補をTabキーで選択する場合も、選択中resultを `scrollIntoView({ block: "nearest", inline: "nearest" })` で表示範囲へ追従させます。
...
`mv`、`rename` は実行前に変更内容をpreviewします。
...
- `mv`、`rename` の確認が不足している場合は `confirmation_required` を返す
