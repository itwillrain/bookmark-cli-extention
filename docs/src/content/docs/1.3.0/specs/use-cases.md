...
対象はhot key、コマンド履歴、readline風入力編集、候補補完、結果表示、pipe filter、clipboard copyです。
...
12. `copy 1` で直前結果のURLまたはpathをclipboardへcopyする
13. `pwd | copy` で現在pathをclipboardへcopyする
14. 必要に応じて `clear` でscrollback transcriptを消す
15. 空promptで `Ctrl+d` を押してCLI windowを閉じる
16. Powerline風promptとplainな結果一覧で実行結果を読む
...
8. ユーザーがPopupでcommand aliasを追加、編集、削除する
...
12. ユーザーが疑似CLIで `abbr j=jump` を実行する
13. 疑似CLIがabbreviationを `settings.commandAbbreviations` に保存する
14. ユーザーが `j ` と入力する
15. 疑似CLIが入力欄の先頭command tokenを `jump ` へ展開する
16. ユーザーが `jump Work` をEnterで確定する
17. 疑似CLIが展開後commandを表示、実行、履歴保存する
18. ユーザーが疑似CLIで `unalias g` または `unabbr j` を実行する
19. 疑似CLIが対応する設定を削除して保存する

完了条件は、Popupで現在のhot keyを確認できることです。

ブラウザ標準のshortcut管理UIへ移動でき、Popupと疑似CLIの両方からcommand aliasを保存できることも含めます。

疑似CLIからcommand abbreviationを保存できることも完了条件です。
